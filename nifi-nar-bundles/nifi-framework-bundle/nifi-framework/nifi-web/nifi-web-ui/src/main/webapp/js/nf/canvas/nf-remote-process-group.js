...
(function (root, factory) {
    ...
    var updateProcessGroupStatus = function (updated) {
        ...
        updated.select('text.remote-process-group-transmission-status')
            .text(function (d) {
                var icon = '';
                if (d.permissions.canRead || d.operatePermissions.canRead) {
                    if (hasIssues(d)) {
                        icon = '\uf071';
            ...
            .classed('invalid', function (d) {
                return (d.permissions.canRead || d.operatePermissions.canRead) && hasIssues(d);
            })
            .classed('transmitting', function (d) {
                return (d.permissions.canRead || d.operatePermissions.canRead) && !hasIssues(d) && d.component.transmitting === true;
            })
            .classed('not-transmitting', function (d) {
                return (d.permissions.canRead || d.operatePermissions.canRead) && !hasIssues(d) && d.component.transmitting === false;
            })
            .each(function (d) {
                // get the tip
                ...
                // if there are validation errors generate a tooltip
                if ((d.permissions.canRead || d.operatePermissions.canRead) && hasIssues(d)) {
                    // create the tip if necessary
                    if (tip.empty()) {
                        tip = d3.select('#remote-process-group-tooltips').append('div')
                            .attr('id', function () {
