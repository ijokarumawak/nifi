...
public class HttpNotificationService extends AbstractNotificationService {
    ...
    protected void init(final NotificationInitializationContext context) {
        ...
        if (url.toLowerCase().startsWith("https")) {
            try {
                ...
                // FIXME search for X509TrustManager instead of explicit cast
                okHttpClientBuilder.sslSocketFactory(sslSocketFactoryWithTrustManagers.getKey(), (X509TrustManager)sslSocketFactoryWithTrustManagers.getValue()[0]);
            } catch (Exception e) {
    ...
    private static Tuple<SSLSocketFactory, TrustManager[]> getSslSocketFactory(NotificationInitializationContext context) throws Exception {
